...
import vibe.core.core : runEventLoopOnce, runTask, yield;
...
class Player
{
    ...
    void readerTask(scope TCPConnection conn)
    {
        try
        {
            ...
            logDebug!"Reader task interrupted";
        catch (Exception e)
            logError!"Exception in reader task: %s"((() @trusted => e.toString)());
        logInfo!"Client %s disconnected"(conn.remoteAddress);
    }
    ...
    void writerTask(scope TCPConnection conn)
    {
        try
            ...
            logDebug!"Writer task interrupted";
        catch (Exception e)
            logError!"Exception in writer task: %s"((() @trusted => e.toString)());
    }
    ...
    void handleRawPacket(InputStream input)
    {
        ...
        {
            logWarn!"Dropping packet for unimplemented protocol %s in state %s"(
        ...
        {
            logWarn!"Dropping packet for unknown protocol %02x in state %s"(
                protocol, m_state,
            );
        }
        ...
        void handleProtocol(alias ct_protocol)()
        {
            static if (is(getPacketImplForProtocolMember!ct_protocol Packet))
            {
                ...
                logDebug!"Got a %s"(packet.stringof);
                handlePacket(packet);
            }
            else
    ...
    private
    void handlePacket(packets.status.client.StatusRequestPacket packet)
    {
    ...
    {
        logDebug!"userName = %s"(packet.getUserName);
        logDebug!"uuid = %s"(packet.getUuid);
    ...
    {
        logDebug!"Channel: %s"(packet.getChannel);
        logDebug!"Data: %s"(packet.getData);
    }

    private
    void handlePacket(packets.config.client.ClientInfoPacket packet)
    {
        logDebug!"ClientInfo: %s"((() @trusted => packet.toString)());
    }
    ...
    {
        m_state = state;
        logDebug!"Switched to state %s"(m_state);
    }
